...
---------------------------------- Link for the challenge: https://codeforces.com/problemset/problem/1695/A -----------------------------

Michael and Joe are playing a game. The game is played on a grid with n rows and m columns, filled with distinct integers. We denote the square on the i-th (1≤i≤n) row 
and j-th (1≤j≤m) column by (i,j) and the number there by aij.
...
Because Michael doesn't like big numbers, he wants the area of the chosen subrectangle (that is, h⋅w
...
It can be shown that Michael can always choose h,w
...
Each of the following n lines contains m integers. The j-th integer on the i-th line is aij (−10^9 ≤ aij ≤ 10^9)  — the element in the cell (i,j).
